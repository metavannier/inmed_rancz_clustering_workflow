...
```{r loadData, echo=FALSE, warning=TRUE, message=TRUE}
...
Perform a Shapiro-Wilk test to check and print whether each numeric column in t_data_final follows a normal distribution (n = 55)

```{r normality, echo=FALSE, warning=TRUE, message=TRUE}
...
```{r anova, echo=FALSE, warning=TRUE, message=TRUE}
...
```{r famd, echo=FALSE, warning=TRUE, message=TRUE}
data <- t_data_final
# write.table(data, "./data_table.txt", sep = "\t", row.names = TRUE, col.names = TRUE, quote = FALSE)
...
# X1sup <- data[,1,drop=FALSE]
# res.sup <- supvar(res.pcamix,X.quanti.sup = NULL, X.quali.sup = X1sup,rename.level=TRUE)

print(res.pcamix$quali$contrib)


# print(res.sup$quanti$cos2[,1:2])
# print(res.sup$levels.sup$cos2[,1:2])
...
fviz_add(p, res.pcamix$quali$contrib,
         col.var = 'cos2')
